...
class RateMyProfScraper:
        ...
        def SearchDepartmentAndInsertProf(self, Department):
            mydb = mysql.connector.connect(
              user="mytestuser", password="mypassword",host="127.0.0.1",
              database='icsteacher', auth_plugin='mysql_native_password'
            )
            ...
            for i in range(0, len(self.professorlist)):
                if(Department == self.professorlist[i]['tDept']):

                    page = requests.get("http://www.ratemyprofessors.com/ShowRatings.jsp?tid=" + str(self.professorlist[i]['tid']))
                    tree = html.fromstring(page.content)

                    courses = tree.xpath('//div[@span="icon-title"]')
                    print(courses.attrib['value'])

                    # parse grade (difficulty, take again) from html
                    # grade[0]: overall quality, grade[1]: would take it again, grade[2]: level of difficulty
                    # grades = tree.xpath('//div[@class="grade"]/text()')
                    # print(grades)
                    # extractGrade = []
                    # for grade in grades:
                    #     s = ""
                    ...
                    #         if c in want:
                    #             s += c;
                    #     extractGrade.append(s)
                    # print(extractGrade)

                    # rating = None if (len(extractGrade) == 0 or extractGrade[0] == "") else extractGrade[0]
                    # takeAgain = None if (len(extractGrade) == 0 or extractGrade[1] == "") else extractGrade[1]
                    # difficulty = None if (len(extractGrade) == 0 or extractGrade[2] == "") else extractGrade[2]

                    # statement_csProf = ("INSERT INTO profRatings (tid, tFname, tLname, tNumRating, overall_Rate, take_again, difficulty) VALUES(%s, %s, %s, %s, %s, %s, %s)")

                    # param = (self.professorlist[i]['tid'], self.professorlist[i]['tFname'], 
                    #     self.professorlist[i]['tLname'], self.professorlist[i]['tNumRatings'], rating, takeAgain, difficulty)
                    # cursor.execute(statement_csProf, param)
                    # mydb.commit()  
...
UCI.SearchDepartmentAndInsertProf("Computer Science")
# UCI.SearchProfessor("Chen Li")
# UCI.PrintProfessorDetail("overall_rating")
